# SOLID-S/high-fan-out-type

Heuristic check for **high fan out type** smells in Go code.

Example: scan the package directory and review the reported evidence before suppressing with `//solidify:ignore SOLID-S/high-fan-out-type reason`.
...
```go
// Positive: the focused positive fixture for SOLID-S/high-fan-out-type crosses the documented signal.
