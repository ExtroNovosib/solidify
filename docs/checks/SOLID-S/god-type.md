...
Heuristic check for **god type** smells in Go code.

Example: scan the package directory and review the reported evidence before suppressing with `//solidify:ignore SOLID-S/god-type reason`.
...
```go
// Positive: the focused positive fixture for SOLID-S/god-type crosses the documented signal.
