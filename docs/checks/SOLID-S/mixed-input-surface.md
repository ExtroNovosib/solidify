# SOLID-S/mixed-input-surface

Heuristic check for **mixed input surface** smells in Go code.

Example: scan the package directory and review the reported evidence before suppressing with `//solidify:ignore SOLID-S/mixed-input-surface reason`.
...
```go
// Positive: the focused positive fixture for SOLID-S/mixed-input-surface crosses the documented signal.
