# SOLID-L/nil-embedded-interface

Heuristic check for **nil embedded interface** smells in Go code.

Example: scan the package directory and review the reported evidence before suppressing with `//solidify:ignore SOLID-L/nil-embedded-interface reason`.
...
```go
// Positive: the focused positive fixture for SOLID-L/nil-embedded-interface crosses the documented signal.
