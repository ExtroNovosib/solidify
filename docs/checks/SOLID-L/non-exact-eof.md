# SOLID-L/non-exact-eof

Heuristic check for **non exact eof** smells in Go code.

Example: scan the package directory and review the reported evidence before suppressing with `//solidify:ignore SOLID-L/non-exact-eof reason`.
...
```go
// Positive: the focused positive fixture for SOLID-L/non-exact-eof crosses the documented signal.
