# SOLID-O/closed-factory

Heuristic check for **closed factory** smells in Go code.

Example: scan the package directory and review the reported evidence before suppressing with `//solidify:ignore SOLID-O/closed-factory reason`.
...
```go
// Positive: the focused positive fixture for SOLID-O/closed-factory crosses the documented signal.
