# SOLID-D/hidden-construction

Heuristic check for **hidden construction** smells in Go code.

Example: scan the package directory and review the reported evidence before suppressing with `//solidify:ignore SOLID-D/hidden-construction reason`.
...
```go
// Positive: the focused positive fixture for SOLID-D/hidden-construction crosses the documented signal.
