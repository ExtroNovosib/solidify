# SOLID-D/infra-error-leak

Heuristic check for **infra error leak** smells in Go code.

Example: scan the package directory and review the reported evidence before suppressing with `//solidify:ignore SOLID-D/infra-error-leak reason`.
...
```go
// Positive: the focused positive fixture for SOLID-D/infra-error-leak crosses the documented signal.
