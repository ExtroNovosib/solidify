# SOLID-D/transport-leak

Heuristic check for **transport leak** smells in Go code.

Example: scan the package directory and review the reported evidence before suppressing with `//solidify:ignore SOLID-D/transport-leak reason`.
...
```go
// Positive: the focused positive fixture for SOLID-D/transport-leak crosses the documented signal.
