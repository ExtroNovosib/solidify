...
func TestPackageCacheKeyIgnoresExecutionOnlySettings(t *testing.T) {
	...
	if newPackageCache("root", base, plan).config != newPackageCache("root", debug, plan).config {
		t.Fatal("cache diagnostics or location changed the cache key")
	}
	changed := base
	changed.MaxInterfaceMethods++
	if newPackageCache("root", base, plan).config == newPackageCache("root", changed, plan).config {
